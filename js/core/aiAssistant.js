  ...
  askQuestion({ module, question, lessonData }) {
    ...
    if (module === "arduino") {
      ...
      if (normalized.includes("오류") || normalized.includes("안돼") || normalized.includes("안 돼")) {
        return "오류를 찾을 때는 회로와 코드를 나누어 보면 좋습니다. D13, 저항, LED 긴 다리, GND 연결 중 어디까지 확인했나요? 코드에서는 pinMode(13, OUTPUT);와 digitalWrite(13, HIGH);가 모두 있는지도 살펴보세요.";
      }
      if (normalized.includes("저항") || normalized.includes("220")) {
        return "저항은 LED에 너무 큰 전류가 흐르지 않도록 도와줍니다. 만약 저항이 없다면 LED와 아두이노 핀에 어떤 일이 생길 수 있을지 예상해볼까요?";
      }
      return "좋은 질문입니다. 바로 답을 말하기보다 코드와 회로를 연결해봅시다. pinMode는 핀의 역할을 정하고, digitalWrite의 HIGH는 어떤 전기 상태를 만든다고 생각하나요?";
    }
